...
int main(int argc, char **argv) {
  ...
  // ".png");
  Init::Feature Temp("../pictures/meter_template/tempdia_V.png");
  ...
  cv::circle(mask_for_dif, cv::Point(diff_tmp.cols / 2, diff_tmp.rows / 2),
  ...
  if (record) {
    ofs << it << ',' << Result.value << ',' << Result.dist << ','
        << Result.readability << ',' << white_num << ',' << std::endl;
  }
