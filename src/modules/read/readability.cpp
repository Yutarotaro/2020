...
result pointerDetection(cv::Mat src, cv::Mat origin) {
  ...
  auto y = [=](int x) -> int { return slope * (x - pt1.x) + pt1.y; };
      ...
      if (origin.at<unsigned char>(cvRound(y(i)), i)) {
        ...
        if (conti_count >= conti_thre) {
          ...
          for (int j = i - conti_count; j <= i; ++j) {
            cv::circle(bgr_origin, cv::Point(j, cvRound(y(j))), 2,
                       cv::Scalar(71, 173, 112), -1, cv::LINE_AA);
          }
        }
        conti_count = 0;
      }
  ...
  //    std::cout << "画像の大きさ: " << origin.rows << ' ' << origin.cols <<
  //    std::endl;
  ...
  //中心からより離れている方が先端←やめた
  // cv::Point tip = (std::abs(conti_tl.x - center.x) > std::abs(conti_br.x -
  // center.x) ? conti_tl : conti_br);
  //左右で白の個数が多い方
  cv::Point tip = (num_tl > num_br ? conti_tl : conti_br);
  std::cout << "num_tl: " << num_tl << "num_br: " << num_br << std::endl;
  cv::circle(bgr_origin, conti_br, 4, cv::Scalar(160, 48, 112), -1,
             cv::LINE_AA);
  cv::circle(bgr_origin, conti_tl, 4, cv::Scalar(160, 48, 112), -1,
             cv::LINE_AA);
  ...
  value = params[meter_type].front_value +
          params[meter_type].k * (rad - params[meter_type].front_rad);

  //    cv::imwrite("./reading/no" + std::to_string(it) + (type ? "pointer" :
  //    "normal") + "_" + std::to_string(value) + ".png", ret);
  ...
  auto dist = [=]() -> double {
    return (lines.size()
                ? std::abs(line_center - center.y) /
                      (double)std::sqrt(1 + slope * slope)
                : 100.);
  };
  ...
  std::cout << "size" << origin.cols << std::endl;

  int white_num = cv::countNonZero(origin);
  if (dist() > readable_thre || white_num < white_thre_min ||
      white_num > white_thre_max) {
