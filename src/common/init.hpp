  ...
  cv::Mat getHomography(Feature &obj) {
    ...
    for (size_t i = 0; i < knnmatch_points.size(); ++i) {
      ...
      if (distance1 <= distance2 * match_par) {
        ...
        match_point1.push_back(
            this->keypoints[knnmatch_points[i][0].queryIdx].pt);
        match_point2.push_back(
            obj.keypoints[knnmatch_points[i][0].trainIdx].pt);
      }
    }
    ...
    for (int i = 0; i < inlierMatch.size(); ++i) {
      ...
      // this->featurePoint.push_back(cv::Point(this->keypoints[kp2_idx].pt.x,
      // this->keypoints[kp2_idx].pt.y));
