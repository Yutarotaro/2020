...
int main(int argc, char** argv)
{
    ...
    cv::Mat Base_clock_tmp = cv::imread("../pictures/" + params[meter_type].picdir + "/pic" + params[meter_type].index + ".JPG", 1);


    //2158,1383
    //文字盤以外にマスクをかける処理
    cv::Mat mask = cv::Mat::zeros(Base_clock_tmp.rows, Base_clock_tmp.cols, CV_8UC1);
    cv::circle(mask, params[meter_type].center, params[meter_type].radius, cv::Scalar(255), -1, CV_AA);

    cv::Mat Base_clock;  //メータ領域だけを残した基準画像
    Base_clock_tmp.copyTo(Base_clock, mask);

    cv::Rect roi_temp(params[meter_type].tl, cv::Size(params[meter_type].l, params[meter_type].l));  //基準画像におけるメータ文字盤部分の位置
    temp = Base_clock(roi_temp);

    ///////////////////////////////

    cv::imwrite("../pictures/meter_template/Base_clock" + meter_type_s + ".png", Base_clock);
