...
int main(int argc, char **argv) {
  ...
  // cv::Mat Base_calib = cv::imread("../pictures/meter_experiment/pic-1.JPG",
  // 1);
  cv::Mat Base_calib = cv::imread("../pictures/dia_experiment_V/pic0.JPG", 1);
  // cv::Mat Base_clock_tmp =
  // cv::imread("../pictures/meter_experiment/pic-2.JPG", 1);
  cv::Mat Base_clock =
  ...
  for (int i = st; i <= to; i++) {
    ...
    std::string path_calib =
        "../pictures/dia_experiment_V/pic" + std::to_string(i) + ".JPG";

    // mask image
    std::string path_clock =
        "../pictures/dia_experiment_V/mask/pic" + std::to_string(i) + ".png";
    ...
    // cv::Mat calib_pos = p.orientation.inv() * p.position -
    // q.orientation.inv() * q.position;
    ...
    Init::Feature ROI("../pictures/dia_experiment_V/roi/pic" +
                      std::to_string(i) + ".png");
    cv::Mat roi = cv::imread(
        "../pictures/dia_experiment_V/roi/pic" + std::to_string(i) + ".png", 1);
    ...
    for (auto j : R_ROI.featurePoint) {
      ROI.featurePoint.push_back(cv::Point(
          j.x / rate + ROI_tl[i].x,
          j.y / rate +
              ROI_tl[i].y)); // resized_roi内座標をNow_clock内座標に変換
    }
    ...
    ofs << i << ',' << cv::norm(distance) << ','
        << cv::norm(calib_pos - r.position) << ','
        << cv::norm(calib_pos - r.position) / cv::norm(calib_pos) << ','
        << cv::norm(r.orientation) * 180.0 / CV_PI << ','
        << cv::norm(rvec) * 180.0 / CV_PI << ',' << error * 180.0 / CV_PI << ','
        << std::endl;
    //}
