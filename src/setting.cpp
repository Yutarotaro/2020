...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < IMAGE_NUM; i++) {
    ...
    // ostr << filepath << "/pictures/dia_experiment_V/calib/pic" << i <<
    // ".JPG";
    ostr << "../pictures/dia_experiment_V/calib/pic" << i << ".JPG";
    // ostr << filepath << "/pictures/meter_experiment/calib/pic" << i <<
    // ".JPG";
  ...
  cv::calibrateCamera(obj_points, img_points, srcImages[0].size(), cam_mat,
                      dist_coefs, rvecs, tvecs);
  ...
  for (int i = IMAGE_NUM - 2; i < IMAGE_NUM; i++) {
    ...
    std::cout << R_tmp.inv() * (-tvecs[i]) << std::endl << std::endl;
  }
